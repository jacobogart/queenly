export function search(searchQuery, bars, queens) {
  ...
  const results = [barResults, queenResults, showResults].flat();
...
export function searchBars(searchQuery, bars) {
  ...
  let matchingBars = bars.filter(bar => 
...
export function searchQueens(searchQuery, queens) {
  const results = [];
  let query = searchQuery.toLowerCase();
  let matchingQueens = queens.filter(queen => 
    queen.name.toLowerCase().includes(query));
